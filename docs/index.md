---
layout: default
title: 项目入口
---

# 秸秆还田与玉米气候损失缓冲分析

本页面提供不含具体数据的项目版本说明、公开研究结果和复现入口。原始数据、中间数据、模型对象、日志、图片和二进制分析产物不通过 GitHub 发布。

## 入口

- [GPT网页端：三个实证方向、37篇文献与外部审阅统一入口](results/report-sr-method-portfolio-web-reader-v1/report.md)
- [版本体系与当前分析入口](VERSIONING.md)
- [逐版本数据、方法与结果呈现变化](VERSION_CHANGELOG.md)
- [版本总表](VERSION_MAP.md)
- [数据谱系](DATA_LINEAGE.md)
- [当前数据资产、面板与实证入口](DATA_ASSET_INVENTORY.md)
- [对话与修改证据](CONVERSATION_EVIDENCE.md)
- [Git 前版本重建缺口](PRE_GIT_RECONSTRUCTION_GAPS.md)
- [公开结果索引](results/)
- [GGCP10 区域优先 scale 搜索](results/scale-search-region-first/report.md)
- [G185 方法升级结果](results/g185-method-upgrade/report.md)
- [G185 旧方法区域 IE/DE/TE 修正版](results/g185-old-method-corrected/report.md)
- [G185 响应面 v3 审阅摘要](results/g185-response-surface-v3/report.md)
- [G185 区域连续灌溉边界](results/g185-region-irrigation-boundary/report.md)
- [数据来源与不公开数据说明](DATA_SOURCES_CN.md)
- [变量字典](VARIABLES.md)

代码仓库位于 [Hazard-cn/CA_soil_moisture](https://github.com/Hazard-cn/CA_soil_moisture)。
