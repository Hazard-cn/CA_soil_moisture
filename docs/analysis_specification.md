...
> **范围说明**: 本文件记录历史 V1/V4/V6 联合胁迫框架，不是当前 G185 区域 IE/DE/TE 结果的真值入口。当前 G185 口径、修正版数值与复现命令见 `quality_reports/handoffs/2026-06-21_g185_method_upgrade_report_handoff.md`。
...
- **数据文件**: `C:/YangSu/00_Project/CA_mechanism/data/master/data_v1_with_climate.csv` (69,038 行, 143 列, GBK 编码；项目外部共享路径)
...
| **Type III: Pathway** | SR→SM 改善 + SM→减缓产量损失 方向是否一致？ | 三链机制检验 + 两方程代数分解 | Step 4v2, Step 4 formal |
...
| Step 4 Formal | `step4_mediation_formal.do` | 两方程组件分解 | Type III |
...
### 研究问题
SR×极端气候交互效应是否与当期条件关联一致，而非由明显的提前项相关所驱动？
...
## 6. [Step 4 Formal] 两方程代数分解 (Type III 补充)

### 研究问题
在给定的 SM 方程与产量方程下，SR×D 和 SR×D×H 对应的乘积组件是否偏离零？这些组件只用于代数分解，不识别因果中介效应。
...
> **文档维护**: 本说明书与历史报告 `output/reports/complete_report_v6.Rmd` (v6.1) 对应。G185 分支更新只写入当前 G185 交接，不在本文件中覆盖历史方程与结果。
