...
> **2026-07-18阅读更新。** 本文是2026-07-15内部候选记录，后续外部方法审阅仍有未解决Major，当前不是投稿候选。“P75−P25差值”和低、高SR端点斜率必须分开解释：year FE下干旱满足损失减弱的统一区间条件，高温的高SR端点区间跨0，热干为`+0.174%→+1.322%`的正向增强点估计；province×year FE下三个胁迫的端点和差值均未形成精确方向性结论。因此，本文摘要中的“部分胁迫损失斜率较平缓”不能作为跨规格统一结论。请先阅读[GitHub网页端统一入口](../report-sr-method-portfolio-web-reader-v1/report.md)、[全国端点斜率表](../report-sr-method-portfolio-web-reader-v1/portfolio/g185_endpoint_slopes.md)和[条件斜率分类规则](../report-sr-method-portfolio-web-reader-v1/methods/claim_classification_rules.md)，再使用本文的完整历史结果。

- Canonical ID：`g185-old-method-unified-override-v1`
- 文稿类型：中文实证小稿
- 历史内部审查状态：`PASS_ROUND2`，90/100；后续外部方法审阅发现未解决Major，当前不是投稿候选
...
极端干旱、高温和热干复合暴露可能改变玉米产量，而秸秆还田采用强度是否对应不同的胁迫条件斜率，以及这种关联是否与根区土壤水分的同期变化一致，仍缺少统一的全国与产区证据。本研究使用中国0.1°网格的2016—2019年G185面板，保留46,299个grid-years和13,236个grids，估计grid固定效应加year固定效应的历史线性两方程，并以grid固定效应加province×year固定效应、2°空间块wild bootstrap及100—300 km空间HAC进行核验。全国样本中，当胁迫取P90、秸秆还田采用强度由P25增加至P75时，干旱、高温和热干分别转换的代数TE差值为0.577%（95%区间0.181%—0.972%）、1.158%（0.753%—1.621%）和1.146%（0.660%—1.696%）。端点拆分后，year FE干旱满足损失减弱的统一区间条件；高温的P75端点区间跨0；热干两端点均为正的点估计，但区间均跨0。province×year FE下三个胁迫的端点和差值区间均未形成精确方向性结论。分别转换的同期土壤水分IE端点幅度小于对应DE端点，但不能解释为IE占TE的比例，更不能据此建立土壤水分因果中介。历史区域规格中的校正TE及其更强固定效应结果继续完整披露。当前证据只能支持按端点和推断层级区分的条件斜率差异，不支持统一全国缓冲、稳定因果效应或已识别土壤水分中介机制。

关键词：秸秆还田；玉米产量；干旱；高温；热干事件；土壤水分；空间推断

## 一、研究问题与文献定位

本文检验三个相互关联但不等同的问题。第一，在既定G185样本中，较高观测秸秆还田采用强度是否对应不同的干旱、高温和热干产量条件斜率。第二，该条件关联是否在东北、黄淮海、西北、西南和华南五个玉米产区之间不同。第三，扩展物候窗口GLEAM根区土壤水分方程产生的同期代数组件与TE差异如何对应。
...
令`M=gleam_smrz_mean_raw`，单位为m³/m³，窗口为项目定义的扩展物候窗口`v3_doy-30`至`ma_doy`；令共同控制向量`Z=(pr_sum_raw, et0_sum_raw, gdd_10_30_raw, irr_frac_raw, aridity_raw)`。对每个胁迫`k`，同期两方程写为：
...
扩展物候窗口GLEAM SMrz与胁迫和产量同期，可能位于胁迫之后。因而IE只是上述同期两方程的代数组件，DE只是控制同期SM后的剩余斜率组件。历史上曾被写成TE的1.50%、3.27%和2.56%实际分别是东北干旱、黄淮海高温和黄淮海热干的DE；本文只把1.850%、3.168%和2.425%称为相应校正TE。
...
[图1“全国不同SR分位点下的同期两方程代数组件”见同目录自包含历史HTML](report.html)。
...
[图2“三个核心区域与胁迫组合的旧线性连续曲线”见同目录自包含历史HTML](report.html)。
...
[图3“五区IE、DE与TE完整对照”见同目录自包含历史HTML](report.html)。
...
历史grid wild、province×year Rademacher和Webb三个推断层的全部45行`p_raw/p_RW/p_Holm`均保留在本地审计表`table5_complete_region_joint_tests.csv`，未按显著性删行；该机器表按仓库政策不进入Git。
...
[`export_g185_old_method_override.py`](../../../scripts/python/export_g185_old_method_override.py)不是模型重估入口。它锁定已审查来源`full_manifest.json`哈希`3a23e22b421f052a4afc5f83c6aa806dd5a8f081bdb83aded357b2af6ca8aaec`，逐字节验证20个登记输出后，只生成override专属六表三图。完整派生机器表、图和`override_manifest.json`保存在不进入Git的本地run中；公开图已内嵌于[自包含历史HTML](report.html)，输出身份与哈希仍由本地manifest审计。原`g185-old-method-unified-v1`的`FULL_STOP`状态没有被删除或覆盖。

## 八、结论与投稿匹配

在G185冻结样本和历史线性估计量下，全国year FE结果只能区分为干旱的统计确认损失减弱、高温的未精确损失减弱点估计和热干的未精确正向增强点估计；更强province×year固定效应下三个胁迫均未形成精确方向性结论。产区结果主要体现高温异质性，东北干旱则表现为正向差值但空间稳定性不足。分别指数转换的同期土壤水分IE端点幅度小于对应DE端点，但三类百分比端点不能作份额分解，也不能支撑“土壤水分已被识别为主要中介”的结论。
