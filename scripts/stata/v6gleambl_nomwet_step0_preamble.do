* =============================================================================
* v6gleambl_nomwet_step0_preamble.do
* Purpose: Build analysis-ready data for four-window GLEAM baseline rerun
*          without paired M_wet mediator controls.
* =============================================================================
...
log using "$logdir/v6gleambl_nomwet_step0_preamble.log", replace

use "$v6nbase_dta", clear
xtset grid_id year
...
forvalues m = 1/3 {
    ...
    forvalues w = 1/4 {
        ...
        gen byte v6n_`tag' = (main_sample == 1) if !missing(main_sample)
        replace v6n_`tag' = 0 if missing(v6n_`tag')
        ...
        foreach v in ln_yield ca `d_var' `w_var' `srd_var' $V6_CTRL_FULL maize_zone irr_group {
            replace v6n_`tag' = 0 if missing(`v') & v6n_`tag' == 1
        }
        ...
        forvalues s = 1/2 {
            ...
            gen double v6n`tag'_`src' = `vdry'
            label var v6n`tag'_`src' "v6 nomwet dry mediator `tag' / `src'"
            replace v6n_`tag' = 0 if missing(v6n`tag'_`src') & v6n_`tag' == 1
        }

        quietly count if v6n_`tag' == 1
        post `pf' ("N_v6n_`tag'") ("`r(N)'")
    }
}
...
preserve
use `diag', clear
export delimited using "$v6ndiag_csv", replace
restore

compress
save "$v6nready_dta", replace

log close
di "=== v6gleambl_nomwet_step0_preamble.do COMPLETE ==="
